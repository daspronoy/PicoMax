...
/*
    ...
    $Vg = \sum_{i=1}^{N_{ATOM}} V_{ig} exp(1i*dot(t_i,g)) / N_{ATOM}$
...
void empiricalpseudopotentialmethod(env &dat){
    ...
    for (int q=0; q<NQ; q++){
        ...
        // Hamiltonian
        Eigen::MatrixXcd H = HamiltonianEPM(dat.lat.G,dat.lat.Q[q],dat.lat.t_vec,dat.mat);
...
void setRefEnergy(env &dat){
    ...
    Eigen::Vector3d K_REF = dat.lat.refpoint;// reference kpoint, where the bandgap occurs

    Eigen::MatrixXcd H = HamiltonianEPM(dat.lat.G,K_REF,dat.lat.t_vec,dat.mat);
