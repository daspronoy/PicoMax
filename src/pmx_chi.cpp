        ...
        for (int i=0; i<NTSR; i++){for (int j=0; j<NTSR; j++){for (int m=0; m<NEPS; m++){for (int n=0; n<=m; n++){
            ...
            for (int f=0; f<NFREQ; f++){
                ...
                if (f==0){
                    ...
                    if (i==0 && j==0){
                        tmp_imag_1 *= SCALEFACTOR / (vecqgm.norm()*vecqgn.norm());
                        tmp_real_1 *= SCALEFACTOR / (vecqgm.norm()*vecqgn.norm());
                    }else if (i==0){
                        tmp_imag_1 *= SCALEFACTOR / (vecqgm.norm()*dat.freq[f]);
                        tmp_real_1 *= SCALEFACTOR / (vecqgm.norm()*dat.freq[f]);
                    }else if (j==0){
                        tmp_imag_1 *= SCALEFACTOR / (vecqgn.norm());
                        tmp_real_1 *= SCALEFACTOR / (vecqgn.norm());
                    }else{
                        tmp_imag_1 *= SCALEFACTOR / (dat.freq[f]*dat.freq[f]);
                        tmp_real_1 *= SCALEFACTOR / (dat.freq[f]*dat.freq[f]);
                    }
                }
                if (dat.kk==0){
                    if (i==0 && j==0){
                        tmp_imag_2 *= SCALEFACTOR / (vecqgm.norm()*vecqgn.norm());
                        tmp_real_2 *= SCALEFACTOR / (vecqgm.norm()*vecqgn.norm());
                    }else if (i==0){
                        tmp_imag_2 *= SCALEFACTOR / (vecqgm.norm());
                        tmp_real_2 *= SCALEFACTOR / (vecqgm.norm());
                    }else if (j==0){
                        tmp_imag_2 *= SCALEFACTOR / (vecqgn.norm());
                        tmp_real_2 *= SCALEFACTOR / (vecqgn.norm());
                    }else{
                        tmp_imag_2 *= SCALEFACTOR;
                        tmp_real_2 *= SCALEFACTOR;
                    }
...
void chi_tensor(env &dat){
    ...
    for (int q=0; q<NQ; q++){
        ...
        for (int k=0; k<NKPT; k++){
            ...
            Eigen::SelfAdjointEigenSolver<Eigen::MatrixXcd> eigsolver_kq(H);
            for (int v=0; v<NBAND; v++){
                E_kq[k][v] = double(eigsolver_kq.eigenvalues()(NBAND-1-c))-dat.energyoffset;
                if (E_kq[k][v]>0) {
                    NBAND_V[k] = v;
                    break;
                }
...
void permittivity(env &dat){
    ...
    double **eigvalue_kq; // eigenvalue at k+q (original comment said k, likely k+q)
    ...
    int NBAND_V [NKPT]; // number of valence bands
    // int NBAND_C [NKPT]; // number of conduction bands // This was commented out, but conceptually needed
    ...
    for (int k=0; k<NKPT; k++){
        ...
        // Hamiltonian at k
        // HamiltonianEPM returns H of size NPWxNPW or 2*NPWx2*NPW if SOC is on
        Eigen::MatrixXcd H = HamiltonianEPM(dat.lat.G,K,dat.lat.atomic,dat.mat);   
        int actual_basis_size = H.cols(); // MODIFIED: Get actual basis size (NPW or 2*NPW for SOC)
        ...
        // MODIFIED: NBAND_V[k] determination logic adjusted for clarity.
        // It now marks the number of bands with energy <= 0, effectively the count of valence bands.
        // The loop iterates up to NBAND to ensure all eigenvalues are processed for this check.
        NBAND_V[k] = 0; // Initialize count of valence bands for current k-point
        ...
        // If all bands are above 0, NBAND_V[k] will be 0.
        // If all bands are below 0, NBAND_V[k] will be NBAND.
        
        eigvector_k[k] = new std::complex<double> *[NBAND]; // MODIFIED: Store all NBAND eigenvectors, not just NBAND_V[k]
        for (int n=0; n<NBAND; n++){ // MODIFIED: Iterate over all computed bands (NBAND)
            eigvector_k[k][n] = new std::complex<double> [actual_basis_size]; // MODIFIED: Use actual_basis_size
            for (int j=0; j<actual_basis_size; j++){ // MODIFIED: Use actual_basis_size
    ...
    eigvalue_kq = new double *[NKPT]; // Allocation strategy for k+q might need review based on usage
    eigvector_kq = new std::complex<double> **[NKPT]; // Allocation strategy for k+q might need review
    for (int k=0; k<NKPT; k++){
        // ointL[k] = new std::complex<double>***[NEPS];
        // ointT[k] = new std::complex<double>***[NEPS];
        eigvalue_kq[k] = new double [NBAND]; // As noted, this implies one (k+q) per k, review needed.
        // If eigvector_kq[k] is to be populated, it would be:
        // eigvector_kq[k] = new std::complex<double>*[NBAND];
        // And then inside the (k,q) loop, for each band n_kq:
        // eigvector_kq[k][n_kq] = new std::complex<double>[actual_basis_size_kq];
        // followed by populating its components.
        // COMMENT: The following section for k+q handling is illustrative and would need actual implementation.
    }
    ...
    for (int q=0; q<NQ; q++){
        ...
        for (int k=0; k<NKPT; k++){
            ...
            // vector{k}
            Eigen::Vector3d K = dat.lat.K[k];
            Eigen::Vector3d K_plus_Q = K + Q; // This needs proper handling for BZ mapping if K points are reduced.

            // Hamiltonian at k+q
            // Eigen::MatrixXcd H_kq = HamiltonianEPM(dat.lat.G, K_plus_Q, dat.lat.atomic, dat.mat);
            // int actual_basis_size_kq = H_kq.cols(); // COMMENT: Get actual basis size for k+q Hamiltonian
            // Eigen::SelfAdjointEigenSolver<Eigen::MatrixXcd> eigsolver_kq(H_kq);
            // if (eigsolver_kq.info() != Eigen::Success){ printf("ERROR: Eigensolver failed for H(k+q)."); abort(); }
            
            
            // int tempindex;
            for (int n=NBAND-1; n>=0; n--){
                // tempindex = NBAND-1-n;
                eigvalue_kq[k][n] = double(eigsolver_kq.eigenvalues()(n))-dat.energyoffset;
