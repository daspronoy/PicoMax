        ...
        for (int i=0; i<NTSR; i++){for (int j=0; j<NTSR; j++){for (int m=0; m<NEPS; m++){for (int n=0; n<=m; n++){
            for (int f=0; f<NFREQ; f++){
                ...
                if (f==0){
                    ...
                    } else if (i%3==0 && j%3==0 && (dat.lat.G[n]).squaredNorm()!=0 && (dat.lat.G[m]).squaredNorm()!=0){
                        tmp_imag_1 *= 1 / ((Q + dat.lat.G[m]).squaredNorm()*(Q + dat.lat.G[n]).squaredNorm());
                        tmp_real_1 *= 1 / ((Q + dat.lat.G[m]).squaredNorm()*(Q + dat.lat.G[n]).squaredNorm());

                        dat.ImXij[q][i][j][m][n][f] = SF*SF_LT * (tmp_imag_1);
                        dat.ReXij[q][i][j][m][n][f] = SF*SF_LT * (tmp_real_1);
                    } else if ( (i%3==0 || j%3==0) && i!=j && (dat.lat.G[n]).squaredNorm()!=0){
                        tmp_imag_1 *= 1 / ((Q + dat.lat.G[n]).squaredNorm()*dat.freq[f]);
                        tmp_real_1 *= 1 / ((Q + dat.lat.G[n]).squaredNorm()*dat.freq[f]);
